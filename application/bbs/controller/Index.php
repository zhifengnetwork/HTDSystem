    ...
    public function thread()
    {
        ...
        if (empty($id)) {
            ...
            if ($a) {
                ...
                $t = $forum->alias('f')->join('forumcate c', 'c.id=f.tid')->join('user m', 'm.id=f.uid', 'left')->field('f.*,c.id as cid,c.alias,c.name,m.id as userid,m.grades,m.point,m.userhead,m.attestation,m.username,m.status')->find($id);
                $this->assign('t', $t);
                if ($t['keywords'] != '') {
